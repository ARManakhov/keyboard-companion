...
class AttrDict(dict):
    """Drop-in replacement for milc.attrdict.AttrDict"""
...
class QFFFontDescriptor:
    ...
    def write(self, fp):
        ...
        fp.write(
            ...
            + o8(
                1 if self.has_ascii_table is True else 0
...
class QFFFont:
    ...
    def _extract_glyphs(self, format):
        ...
        for _, glyph_entry in self.glyph_data.items():
            glyph_img = converted_img.crop(
                (glyph_entry.x, 1, glyph_entry.x + glyph_entry.w, 1 + self.glyph_height)
            )
            (_, this_glyph_image_bytes) = qmk.painter.convert_image_bytes(
                glyph_img, format
            )
            this_glyph_rle_bytes = qmk.painter.compress_bytes_qmk_rle(
                this_glyph_image_bytes
            )
    ...
    def _parse_image(
        self, img, include_ascii_glyphs: bool = True, unicode_glyphs: str = ""
    ):
        ...
        if len(glyph_pixel_offsets) != len(glyphs):
            self.logger.error(
                "The number of glyphs to generate doesn't match the number of detected glyphs in the input image."
            )
            return

        # Set up the required metadata for each glyph
        for n in range(0, len(glyph_pixel_offsets)):
            self.glyph_data[glyphs[n]] = QFFGlyphInfo(
                code_point=glyphs[n], x=glyph_pixel_offsets[n], w=glyph_pixel_widths[n]
            )
    ...
    def generate_image(
        ...
        ttf_file: Path,
        font_size: int,
        include_ascii_glyphs: bool = True,
        unicode_glyphs: str = "",
        include_before_left: bool = False,
        use_aa: bool = True,
    ):
        # Load the font
        ...
        img = Image.new(
            "RGB", (total_glyph_width + 1, max_font_size * 2 + 1), (0, 0, 0, 255)
        )
        cur_x_pos = 0
        ...
        dummy_img = Image.new(
            "RGB", (total_glyph_width + 1, max_font_size + 1), (0, 0, 0, 255)
        )
    ...
    def read_from_image(
        ...
        self._parse_image(
            Image.open(str(img_file)), include_ascii_glyphs, unicode_glyphs
        )
        return
    ...
    def save_to_qff(self, format: Dict[str, Any], use_rle: bool, fp):
        ...
        for _, glyph_entry in self.glyph_data.items():
            ...
            glyph_img_bytes = (
                glyph_entry.image_compressed_bytes
                if use_rle
                else glyph_entry.image_uncompressed_bytes
            )
        ...
        for code_point, glyph_entry in self.glyph_data.items():
            if (
                ord(code_point) >= 0x20
                and ord(code_point) <= 0x7E
                and include_ascii_glyphs
            ):
        ...
        if format["has_palette"]:
            ...
            palette_descriptor.palette_entries = list(
                map(rgb888_to_qmk_hsv888, self.palette)
            )
