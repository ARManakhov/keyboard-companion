...
def cmd_list(args):
    ...
    print(f"{'VID'}  {'PID':>4}  {'IF':>4}  {'Manufacturer':<20}  {'Product':<25}  {'Path'}")
    ...
    for dev in devices:
        ...
        print(f"{vid:04X}  {pid:04X}  {interface:>3}  {manufacturer:<20}  {product:<25}  {path}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Device monitor and controller"
    )
    ...
    parser_connect.add_argument(
        "--vid", type=lambda x: int(x, 0), required=True,
        help="Vendor ID (hex: 0xE126 or decimal: 57638)"
    )
    parser_connect.add_argument(
        "--pid", type=lambda x: int(x, 0), required=True,
        help="Product ID (hex: 0x0051 or decimal: 81)"
    )
    parser_connect.add_argument(
        "--interface", type=int, default=1,
        help="Interface number (by default: 1)"
    )
